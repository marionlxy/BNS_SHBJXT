...
!function (a) {
	...
	var b = {
		...
		validate: function () {
			...
			if (c.is("form") || c.hasClass("validationEngineContainer")) {
				...
				c.is("form") || c.hasClass("validationEngineContainer") ? c.removeClass("validating") : (f = c.closest("form, .validationEngineContainer"), e = f.data("jqv") ? f.data("jqv") : a.validationEngine.defaults, d = b._validateField(c, e), d && e.onFieldSuccess ? e.onFieldSuccess() : e.onFieldFailure && e.InvalidFields.length > 0 && e.onFieldFailure());
		...
		hide: function () {
			var f,
			c = a(this).closest("form, .validationEngineContainer"),
			d = c.data("jqv"),
		...
		_validateFields: function (c) {
			...
			if (c.trigger("jqv.form.validating"), f = null, c.find("[" + d.validateAttribute + "*=validate]").not(":disabled").each(function () {
					...
					if (a.inArray(g.attr("name"), h) < 0) {
						if (e |= b._validateField(g, d), e && null == f && (g.is(":hidden") && d.prettySelect ? f = g = c.find("#" + d.usePrefix + b._jqSelector(g.attr("id")) + d.useSuffix) : (g.data("jqv-prompt-at")instanceof jQuery ? g = g.data("jqv-prompt-at") : g.data("jqv-prompt-at") && (g = a(g.data("jqv-prompt-at"))), f = g)), d.doNotShowAllErrosOnSubmit)
							return !1;
						if (h.push(g.attr("name")), 1 == d.showOneMessage && e)
							return !1
					}
				...
				success: function (e) {
					...
					if ("json" == h && e !== !0) {
						for (f = !1, g = 0; g < e.length; g++)
							i = e[g], j = i[0], k = a(a("#" + j)[0]), 1 == k.length && (l = i[2], 1 == i[1] ? "" != l && l ? (d.allrules[l] && (m = d.allrules[l].alertTextOk, m && (l = m)), d.showPrompts && b._showPrompt(k, l, "pass", !1, d, !0)) : b._closePrompt(k) : (f |= !0, d.allrules[l] && (m = d.allrules[l].alertText, m && (l = m)), d.showPrompts && b._showPrompt(k, l, "", !1, d, !0)));
		...
		_validateField: function (c, d, e) {
			...
			y,
			z;
			if (c.attr("id") || (c.attr("id", "form-validation-field-" + a.validationEngine.fieldIdCounter), ++a.validationEngine.fieldIdCounter), !d.validateNonVisibleFields && (c.is(":hidden") && !d.prettySelect || c.parent().is(":hidden")))
				return !1;
			if (f = c.attr(d.validateAttribute), g = /validate\[(.*)\]/.exec(f), !g)
				return !1;
			for (h = g[1], i = h.split(/\[|,|\]/), j = !1, k = c.attr("name"), l = "", m = "", n = !1, o = !1, d.isError = !1, d.showArrow = !0, d.maxErrorsPerField > 0 && (o = !0), p = a(c.closest("form, .validationEngineContainer")), q = 0; q < i.length; q++)
		...
		_getCustomErrorMessage: function (a, c, d, e) {
			...
			if (void 0 != g && (f = a.attr("data-errormessage-" + g), void 0 != f))
				return f;
			if (f = a.attr("data-errormessage"), void 0 != f)
				return f;
			if (h = "#" + a.attr("id"), "undefined" != typeof e.custom_error_messages[h] && "undefined" != typeof e.custom_error_messages[h][d])
		...
		_required: function (b, c, d, e, f) {
			...
			default:
				if (g = a.trim(b.val()), h = a.trim(b.attr("data-validation-placeholder")), i = a.trim(b.attr("placeholder")), !g || h && g == h || i && g == i)
			...
			case "checkbox":
				...
				if (j = b.closest("form, .validationEngineContainer"), k = b.attr("name"), 0 == j.find("input[name='" + k + "']:checked").size())
					return 1 == j.find("input[name='" + k + "']:visible").size() ? e.allrules[c[d]].alertTextCheckboxe : e.allrules[c[d]].alertTextCheckboxMultiple
			}
		},
		...
		_custom: function (a, b, c, d) {
			...
			if (f.regex) {
				...
				if (g = f.func, "function" != typeof g)
					return alert("jqv:custom parameter 'function' is no function - " + e), void 0;
				if (!g(a, b, c, d))
					return d.allrules[e].alertText
			}
		},
		...
		_ajax: function (c, d, e, f) {
			...
			if (j)
				for (o = String(j).split(","), e = 0; e < o.length; e++)
					p = o[e], a(p).length && (q = c.closest("form, .validationEngineContainer").find(p).val(), p.replace("#", "") + "=" + escape(q), k[p.replace("#", "")] = q);
		...
		_buildPrompt: function (c, d, e, f, g) {
			...
			if (f && h.addClass("ajaxed"), a("<div>").addClass("formErrorContent").html(d).appendTo(h), j = c.data("promptPosition") || g.promptPosition, g.showArrow)
				switch (k = a("<div>").addClass("formErrorArrow"), "string" == typeof j && (l = j.indexOf(":"), -1 != l && (j = j.substring(0, l))), j) {
				case "bottomLeft":
				case "bottomRight":
		...
		_saveOptions: function (b, c) {
			...
			e = a.extend(!0, {}, a.validationEngine.defaults, c),
			b.data("jqv", e),
			e
		},
		...
		_condRequired: function (a, c, d, e) {
			...
			for (f = d + 1; f < c.length; f++)
				if (g = jQuery("#" + c[f]).first(), g.length && void 0 == b._required(g, ["required"], 0, e, !0))
	...
	a(function () {
		a.validationEngine.defaults.promptPosition = b.isRTL() ? "topRight" : "topLeft"
	})
}
(jQuery);
