package com.krm.voteplateform.web.ptsystemsmenu.controller;
 ...
 */
@Controller
public class PtSystemsMenuController extends BaseController {
	 ...
	 * @return
	 */
	@RequestMapping("ptsystems/toMain/{code}")
	public String toMain(HttpServletRequest request, Model model, @PathVariable String code) {
		...
		PtCommission ptCommission = ptCommissionService.getOneCommissionByCode(param);
		sessionLoginUser.setPtCommission(ptCommission);
		// 2.设置logo图片
		...
		logger.info("左侧菜单：{}", JSON.toJSONString(userMenus));
		Map<String, Object> oneUserInfo = SysUserUtils.getOneUserInfo(request);
		String roleCategory = "";
		if (oneUserInfo != null && !oneUserInfo.isEmpty() && oneUserInfo.containsKey("roleCategory")) {
			roleCategory = (String) oneUserInfo.get("roleCategory");
		...
		logger.info("左侧菜单：{}", JSON.toJSONString(userMenus));
		return "ptsystems/ptsystemsMain";
	}
	 ...
	 * @return
	 */
	@RequestMapping("ptsystems/toIndex/{menuCode}")
	public String toIndex(HttpServletRequest request, Model model, @PathVariable String menuCode) {
		...
		if (StringUtils.equals(SPECIAL_MENU_CODE_VOTE, menuCode)) { // 委员端当前会议menu8
			Map<String, Object> findCurrentMeeting = basMettingService.findCurrentMeeting();// 取得当前会议
			if (findCurrentMeeting != null && !findCurrentMeeting.isEmpty()) {// 若存委员端在当前会议
				model.addAttribute(METTING_NAME, findCurrentMeeting.get("specMettingTitle"));// 设定会议名称
				...
				/*
				 * String mettingId = (String) findCurrentMeeting.get("id"); String clientAddress =
				 * IPUtils.getClientAddress(request); Map<String, Object> map = new HashMap<String,
				 * Object>(); map.put("mettingId", mettingId); map.put("IP", clientAddress); //
				 * 向委员会插入中插入与会信息 basMettingMemService.saveBasMettingMem(map);
				 */
			}
		}
		toUrl += menuCode;
	...
	@RequestMapping("ptsystems/toFunctionIndex")
	public String toFunctionIndex(HttpServletRequest request, Model model, String functionCode, String projectId,
			String mettingId) {
		String toUrl = "ptsystems/" + SysUserUtils.getCurrentCommissionCode().toLowerCase() + "/" + functionCode;
		...
		if (StringUtils.isNotEmpty(oneBasFunConf.getSkipClassMethod())) {
			...
			if (!StringUtils.equals("0", projectId)) {
				...
				if (FUNCTION_CODE_ADD_METTING.equals(functionCode)) {
					map = Maps.newHashMap();
					Object exec = irh.exec(map);
					if (exec != null && exec instanceof Map) {
	 ...
	 * @return
	 */
	@RequestMapping("ptsystems/toRoleIndex")
